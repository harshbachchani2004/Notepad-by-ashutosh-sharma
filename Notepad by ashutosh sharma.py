...
def saveasfile():
    global file
    if file==None:
        file=asksaveasfile(initialfile="untited",defaultextension=".txt",filetypes=[("All Files","*.*"),("Pdf Document","*.pdf")])
        if file.read() is empty:
            showerror("Notepad", "file is empty,unable to save")
            file=None
        else:
            file.write(t.get(1.0,END))
...
menubar1.add_command(label="save as",command=saveasfile)
